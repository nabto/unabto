 ...
 * @brief    Nano102/112 series PWM driver header file
 *
 * @note
...
/** @addtogroup NANO1X2_PWM_Driver PWM Driver
  @{
*/

/** @addtogroup NANO1X2_PWM_EXPORTED_CONSTANTS PWM Exported Constants
  @{
*/
#define PWM_CHANNEL_NUM                      (4)      /*!< PWM channel number */
#define PWM_CH_0_MASK                        (1UL)    /*!< PWM channel 0 mask */
#define PWM_CH_1_MASK                        (2UL)    /*!< PWM channel 1 mask */
#define PWM_CH_2_MASK                        (4UL)    /*!< PWM channel 2 mask */
#define PWM_CH_3_MASK                        (8UL)    /*!< PWM channel 3 mask */
...
#define PWM_RISING_LATCH_INT_ENABLE          (1UL)    /*!< PWM rising latch interrupt enable */
#define PWM_FALLING_LATCH_INT_ENABLE         (2UL)    /*!< PWM falling latch interrupt enable */
#define PWM_RISING_FALLING_LATCH_INT_ENABLE  (3UL)    /*!< PWM rising latch interrupt enable */
#define PWM_RISING_LATCH_INT_FLAG            (2UL)    /*!< PWM rising latch condition happened */
#define PWM_FALLING_LATCH_INT_FLAG           (4UL)    /*!< PWM falling latch condition happened */
#define PWM_RISING_FALLING_LATCH_INT_FLAG    (6UL)    /*!< PWM rising latch condition happened */
#define PWM_RISING_LATCH_PDMA_ENABLE         (0x10UL)    /*!< PWM rising latch PDMA enable */
#define PWM_FALLING_LATCH_PDMA_ENABLE        (0x20UL)    /*!< PWM falling latch PDMA enable */
#define PWM_RISING_FALLING_LATCH_PDMA_ENABLE (0x30UL)    /*!< PWM rising and falling latch PDMA enable */
#define PWM_CAP_PDMA_RFORDER_R               (0x1000UL)  /*!< PWM captured data transferred by PDMA is rising latch first */
#define PWM_CAP_PDMA_RFORDER_F               (0UL)       /*!< PWM captured data transferred by PDMA is falling latch first */

/*@}*/ /* end of group NANO1X2_PWM_EXPORTED_CONSTANTS */


/** @addtogroup NANO1X2_PWM_EXPORTED_FUNCTIONS PWM Exported Functions
  @{
*/
...
do { \
    ...
    for (i = 0; i < PWM_CHANNEL_NUM; i++) { \
        if ( u32ChannelMask  & (1 << i)) { \
            u32Mask |= (PWM_CTL_CH0INV_Msk << (i * 8)); \
        } \
    } \
    pwm->CTL |= u32Mask;\
  }while(0)
...
do { \
    ...
    *(__IO uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) |= (u32CMR << PWM_DUTY0_CM_Pos); \
}while(0)

#define PWM_GET_CNR(pwm, u32ChannelNum) (*(__IO uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) & PWM_DUTY0_CN_Msk)
...
do { \
    uint8_t i; \
    pwm->CTL &= ~(PWM_CTL_PWMTYPE01_Msk | PWM_CTL_PWMTYPE23_Msk); \
    for (i = 0; i < PWM_CHANNEL_NUM; i++) { \
        if ( u32ChannelMask  & (1 << i)) \
            pwm->CTL |= (PWM_CTL_PWMTYPE01_Msk << (i >> 1)); \
    } \
} while(0)
...
/*@}*/ /* end of group NANO1X2_PWM_EXPORTED_FUNCTIONS */

/*@}*/ /* end of group NANO1X2_PWM_Driver */
