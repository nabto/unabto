 ...
 * $Revision: 10 $
 * $Date: 14/11/27 7:10p $
 * @brief    Nano 102/112 series LCD driver source file
 *
 * @note
...
/** @addtogroup NANO1X2_LCD_EXPORTED_VARIABLES LCD Exported Variables
  @{
*/
/*---------------------------------------------------------------------------------------------------------*/
...
/*@}*/ /* end of group NANO1X2_LCD_EXPORTED_VARIABLES */
...
uint32_t LCD_Open(uint32_t u32DrivingType, uint32_t u32ComNum, uint32_t u32BiasLevel, uint32_t u32FramerateDiv, uint32_t u32DrivingVol)
{
    ...
    /* IP reset */
    SYS->IPRST_CTL2 |= SYS_IPRSTCTL2_LCD;
    SYS->IPRST_CTL2 &= ~SYS_IPRSTCTL2_LCD;
