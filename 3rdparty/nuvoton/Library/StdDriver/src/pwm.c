 ...
 * $Revision: 15 $
 * $Date: 14/11/27 7:10p $
 * @brief    Nano 102/112 series PWM driver source file
 *
 * @note
                                  ...
                                  uint32_t u32DutyCycle)
{
    ...
    switch ((CLK->CLKSEL1 & (CLK_CLKSEL1_PWM0_CH01_S_Msk << (u32ChannelNum & 2))) >> (CLK_CLKSEL1_PWM0_CH01_S_Pos + (u32ChannelNum & 2))) {
    case 0:
        u32PWM_Clock = __HXT;
        break;
    case 1:
    ...
    while((pwm->INTSTS & PWM_INTSTS_PresSyncFlag_Msk ) == PWM_INTSTS_PresSyncFlag_Msk);
    ...
    *(__IO uint32_t *) (&pwm->DUTY0 + 3 * u32ChannelNum) &= ~PWM_DUTY0_CN_Msk;
...
{
    pwm->ADTRGEN |= (PWM_ADTRGEN_TRGCH0EN_Msk << u32ChannelNum);
}
...
{
    pwm->ADTRGSTS |= (PWM_ADTRGSTS_ADTRG0Flag_Msk << u32ChannelNum);
}
 ...
 * @param[in] pwm The base address of PWM module
 * @param[in] u32ChannelNum PWM channel number. Valid values are between 0~5
 * @return Trigger ADC or not
 * @retval 0 The designated channel did not trigger ADC
 * @retval 1 The designated channel triggered ADC
*/
uint32_t PWM_GetADCTriggerFlag (PWM_T *pwm, uint32_t u32ChannelNum)
{
    return ((pwm->ADTRGSTS & (PWM_ADTRGSTS_ADTRG0Flag_Msk << u32ChannelNum)) >> u32ChannelNum);
}
...
{
    // write 1 clear
    pwm->INTSTS = (PWM_INTEN_TMIE0_Msk << u32ChannelNum);
}
...
{
    return ((pwm->INTSTS & (PWM_INTEN_TMIE0_Msk << u32ChannelNum)) ? 1 : 0);
}
