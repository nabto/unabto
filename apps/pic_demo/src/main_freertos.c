/*
 *  Implementation of main for uNabto SDK on PIC with FreeRTOS
 */
...
// Small bug in FreeRTOS: If ISR_Support.h can't locate
// FreeRTOSConfig.h, you have to specify the whole path
// "../../../demo/pic_demo/src/FreeRTOSConfig.h"
#include "FreeRTOS.h"
#include "task.h"
...
const uint32_t high_mac_addr = 0x0004a3;

// Local function prototypes
void unabto_task( void* params );

// Local function prototypes
static void InitializeBoard(void);

// Make space for nabto specific settings
#pragma udata big_mem_section
static nabto_main_context nmc;
#pragma udata

// <editor-fold defaultstate="collapsed" desc="PIC18 Interrupt Service Routines">
//
...
#endif
{
    ...
    platform_initialize(false);

    if (xTaskCreate(unabto_task, (const signed char*)"unabto main loop", 2048, NULL, 1, NULL) != pdPASS) {
        NABTO_LOG_TRACE(("failed to create unabto task"));
    }
...
/* We are implementing our own tick functions instead of the TCP/IP Stacks */
void TickInit(void) {
    ;
}

DWORD TickGet(void) {
    return xTaskGetTickCount()*128;
}
