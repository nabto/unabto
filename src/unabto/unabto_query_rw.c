...
bool unabto_query_write_list_end(unabto_query_response* queryResponse, unabto_list_ctx* listCtx, uint16_t elementCount) {
    ...
    if (NULL == queryResponse || NULL == listCtx || NULL == *listCtx || UNABTO_ABUFFER_GET_INITIAL_HEAD(queryResponse) > (uint8_t*)*listCtx || UNABTO_ABUFFER_GET_HEAD(queryResponse) - sizeof(uint16_t) < (uint8_t*)*listCtx) {
        return false;
    }
