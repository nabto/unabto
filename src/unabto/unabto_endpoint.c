...
const char* nabto_ip_to_string(const struct nabto_ip_address* addr)
{
    ...
    if (addr->type == NABTO_IP_NONE) {
        snprintf(output, sizeof(output), "NABTO_IP_NONE");
    } else if (addr->type == NABTO_IP_ANY) {
        snprintf(output, sizeof(output), "NABTO_IP_ANY");
    } else if (addr->type == NABTO_IP_V4) {
        uint32_t ip = addr->addr.ipv4;
        snprintf(output, sizeof(output), "%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8, (uint8_t)(ip >> 24), (uint8_t)(ip >> 16), (uint8_t)(ip >> 8), (uint8_t)(ip));
    } else {
        const uint8_t* ip = addr->addr.ipv6;
        snprintf(output, sizeof(output), "%02X%02X:" "%02X%02X:" "%02X%02X:" "%02X%02X:" "%02X%02X:" "%02X%02X:" "%02X%02X:" "%02X%02X", ip[0], ip[1], ip[2], ip[3], ip[4], ip[5], ip[6], ip[7], ip[8], ip[9], ip[10], ip[11], ip[12], ip[13], ip[14], ip[15]);
    }
    return output;
}
