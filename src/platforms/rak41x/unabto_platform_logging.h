...
#define NABTO_LOG_BASIC_PRINT(severity, message)                                                            \
    do {                                                                                                    \
        OS_ERR osErr;                                                                                       \
        OSSemPend(&loggingSemaphore, 0, OS_OPT_PEND_BLOCKING, NULL, &osErr);                                \
        printf("\n%10" PRIu32 " %" PRItext ":%u  ", OSTimeGet(&osErr), clean_filename(__FILE__), __LINE__); \
        printf message;                                                                                     \
        OSSemPost(&loggingSemaphore, OS_OPT_POST_NO_SCHED, &osErr);                                         \
    } while (0)
