...
#define print_log_preamble()                                                                                                   \
    do {                                                                                                                       \
        printf("\n%10" PRIu32 " %" PRItext ":%u  ", TickConvertToMilliseconds(TickGet()), clean_filename(__FILE__), __LINE__); \
    } while (0)
#elif NABTO_ENABLE_LOG_TIMESTAMP
#define print_log_preamble()                                               \
    do {                                                                   \
        printf("\n%10" PRIu32 "  ", TickConvertToMilliseconds(TickGet())); \
    } while (0)
...
#define print_log_preamble()                                          \
    do {                                                              \
        debug_channel_write('\n');                                    \
        debug_channel_write_string_pgm(clean_filename(__FILE__ ":")); \
        debug_channel_write_int32(__LINE__);                          \
        debug_channel_write_string_pgm("  ");                         \
    } while (0)
#else
#define print_log_preamble()                                               \
    do {                                                                   \
        printf("\n%" PRItext ":%u  ", clean_filename(__FILE__), __LINE__); \
    } while (0)
#endif
#else
#define print_log_preamble()       \
