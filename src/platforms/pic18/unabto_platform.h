...
#define critical_section_enter()        \
    uint8_t __gieStore = INTCON & 0xc0; \
    INTCON &= 0x3f;
// Exit a critical section.
#define critical_section_exit() INTCON |= __gieStore;

// Override default read/write macros with PIC18 optimized version.
#define READ_U8(variable, buffer)           \
    do {                                    \
        (variable) = *((uint8_t*)(buffer)); \
    } while (0)
#define READ_U16(variable, buffer)                \
    do {                                          \
        (variable) = swaps(*(uint16_t*)(buffer)); \
    } while (0)
#define READ_U32(variable, buffer)                \
    do {                                          \
        (variable) = swapl(*(uint32_t*)(buffer)); \
    } while (0)
#define WRITE_U8(buffer, variable)          \
    do {                                    \
        *((uint8_t*)(buffer)) = (variable); \
    } while (0)
#define WRITE_U16(buffer, variable)             \
    do {                                        \
        *(uint16_t*)(buffer) = swaps(variable); \
    } while (0)
#define WRITE_U32(buffer, variable)             \
    do {                                        \
        *(uint32_t*)(buffer) = swapl(variable); \
    } while (0)
