...
void tunnel_event(tunnel* tunnel, tunnel_event_source event_source) {
	...
	NABTO_LOG_INFO(("tunnel event IDLE"));
    ...
    if (tunnel->state == TS_READ_COMMAND) {
        NABTO_LOG_INFO(("tunnel event READ_COMMAND"));
	const uint8_t* buf;
    ...
    if (tunnel->state == TS_PARSE_COMMAND) {
        NABTO_LOG_INFO(("tunnel event PARSE_COMMAND"));
	...
	NABTO_LOG_INFO(("tunnel event OPENING_SOCKET && TCP_WRITE"));
	...
	NABTO_LOG_INFO(("tunnel event OPEN_SOCKET"));
	//~ printf("TS_OPEN_SOCKET state\n");
        // TODO make async
	...
	NABTO_LOG_INFO(("tunnel event FORWARD"));
    ...
    if (tunnel->extReadState == FS_CLOSING && tunnel->unabtoReadState == FS_CLOSING) {
	NABTO_LOG_INFO(("tunnel event FS_CLOSING"));
        tunnel->state = TS_CLOSING;
    }


    if (tunnel->state == TS_CLOSING) {
	NABTO_LOG_INFO(("tunnel event TS_CLOSING"));
        const uint8_t* buf;
        unabto_stream_hint hint;
        size_t readen;
...
void tcp_forward(tunnel* tunnel) {
    while(true) {
        if (tunnel->extReadState == FS_READ) {
            ssize_t readen = recv(tunnel->tunnel_type_vars.tcp.sock, tunnel->staticMemory->stmu.tcp_sm.tcpReadBuffer, NABTO_MEMORY_STREAM_SEND_SEGMENT_SIZE, 0);
            if (readen == 0) {
                // eof
            ...
            if (readen > 0) {
                tunnel->tunnel_type_vars.tcp.tcpReadBufferSize = readen;
                tunnel->tunnel_type_vars.tcp.tcpReadBufferSent = 0;
                tunnel->extReadState = FS_WRITE;
            }
        ...
        if (tunnel->extReadState == FS_WRITE) {
            unabto_stream_hint hint;
            size_t written = unabto_stream_write(tunnel->stream, tunnel->staticMemory->stmu.tcp_sm.tcpReadBuffer+tunnel->tunnel_type_vars.tcp.tcpReadBufferSent, tunnel->tunnel_type_vars.tcp.tcpReadBufferSize - tunnel->tunnel_type_vars.tcp.tcpReadBufferSent, &hint);
            if (hint != UNABTO_STREAM_HINT_OK) {
                NABTO_LOG_TRACE(("Can't write to stream"));
                close_tcp_reader(tunnel);
                break;
                ...
                if (written > 0) {
                    tunnel->tunnel_type_vars.tcp.tcpReadBufferSent += written;
                    if (tunnel->tunnel_type_vars.tcp.tcpReadBufferSize == tunnel->tunnel_type_vars.tcp.tcpReadBufferSent) {
                        tunnel->extReadState = FS_READ;
                    }
                } else {
                    break;
