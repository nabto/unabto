...
NABTO_THREAD_LOCAL_STORAGE union tunnel_static_memory* tunnels_static_memory = 0;
...
bool init_tunnel_module()
{
    ...
    tunnels_static_memory = (union tunnel_static_memory*)malloc(sizeof(union tunnel_static_memory) * NABTO_MEMORY_STREAM_MAX_STREAMS);
...
void tunnel_event(tunnel* tunnel, tunnel_event_source event_source) {
    ...
    if (tunnel->state == TS_IDLE) {
        NABTO_LOG_ERROR(("Tunnel(%i), Event on tunnel which should not be in IDLE state. source %i, uartReadState %i, unabtoReadState %i, stream index %i, fd %i", tunnel->tunnelId, event_source, tunnel->tunnel_type_vars.uart.uartReadState, tunnel->tunnel_type_vars.uart.unabtoReadState, unabto_stream_index(tunnel->stream), tunnel->tunnel_type_vars.uart.fd));
        return;
    }
            ...
            if (readen > 0) {
                ...
                for (i = 0; i < readen; i++) {
                    if (buf[i] == '\n') {
                        ...
                        tunnel->staticMemory->uart_sm.command[tunnel->commandLength] = buf[i];
    ...
    if (tunnel->state == TS_PARSE_COMMAND) {
        if (parse_command(tunnel)) {
            ...
            if (open_uart(tunnel)) {
                ...
                NABTO_LOG_INFO(("Tunnel(%i) connecting to %s", tunnel->tunnelId, tunnel->staticMemory->uart_sm.deviceName));
            ...
            NABTO_LOG_ERROR(("Tunnel(%i) Could not parse tunnel command %s", tunnel->tunnelId, tunnel->staticMemory->uart_sm.command));
    ...
    if (tunnel->state == TS_FORWARD) {
        if (tunnel->tunnel_type_vars.uart.uartReadState == FS_CLOSING && tunnel->tunnel_type_vars.uart.unabtoReadState == FS_CLOSING) {
            tunnel->state = TS_CLOSING;
        } else {
            uart_forward(tunnel);
...
bool parse_command(tunnel* tunnel) {
    ...
    strncpy(tunnel->staticMemory->uart_sm.deviceName, uart_tunnel_get_default_device(), MAX_DEVICE_NAME_LENGTH);
    return true;
}
...
void close_uart_reader(tunnel* tunnel) {
    ...
    tunnel->tunnel_type_vars.uart.uartReadState = FS_CLOSING;
}
...
void uart_forward(tunnel* tunnel) {
    while(true) {
        ...
        if (tunnel->tunnel_type_vars.uart.uartReadState == FS_CLOSING) {
            break;
        }
        ...
        if (canWriteToStreamBytes == 0) {
            tunnel->tunnel_type_vars.uart.uartReadState = FS_WRITE;
            break;
        } else {
            tunnel->tunnel_type_vars.uart.uartReadState = FS_READ;
        }
        {
            int readen;
...
void close_stream_reader(tunnel* tunnel) {
    tunnel->tunnel_type_vars.uart.unabtoReadState = FS_CLOSING;
 ...
 */
void unabto_forward(tunnel* tunnel) {
    if (tunnel->tunnel_type_vars.uart.unabtoReadState == FS_WRITE) {
        tunnel->tunnel_type_vars.uart.unabtoReadState = FS_READ;
    }
    while(true) {
        if (tunnel->tunnel_type_vars.uart.unabtoReadState == FS_READ) {
            const uint8_t* buf;
            unabto_stream_hint hint;
            size_t readen = unabto_stream_read(tunnel->stream, &buf, &hint);
            if (hint != UNABTO_STREAM_HINT_OK) {
                ...
                if (readen == 0) {
                    ...
                    if (written > 0) {
                        ...
                        tunnel->tunnel_type_vars.uart.unabtoReadState = FS_WRITE;
                        break;
                    } else { // -1
        ...
        if (tunnel->tunnel_type_vars.uart.unabtoReadState == FS_CLOSING) {
            break;
        }
    }
}
...
bool open_uart(tunnel* tunnel) {
    ...
    tunnel->tunnel_type_vars.uart.fd = open(tunnel->staticMemory->uart_sm.deviceName, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (tunnel->tunnel_type_vars.uart.fd == -1) {
        NABTO_LOG_ERROR(("cannot open uart %s", tunnel->staticMemory->uart_sm.deviceName));
        return false;
    }
	...
	{
		NABTO_LOG_ERROR(("Unable to configure UART '%s'. error: %s", tunnel->staticMemory->uart_sm.deviceName, strerror(errno)));
        return false;
	}
...
void reset_tunnel_struct(tunnel* t) {
    ...
    memset(t->staticMemory, 0, sizeof(union tunnel_static_memory));
    ...
    for(i = 0; i < NABTO_MEMORY_STREAM_MAX_STREAMS; i++) {
        ...
        if (&tunnels[i] != tun) {
            ...
            if (t->state == TS_FORWARD &&
                (strcmp(tun->staticMemory->uart_sm.deviceName, t->staticMemory->uart_sm.deviceName) == 0))
            {
                // close the other tunnels
                NABTO_LOG_INFO(("tunnel %i is stealing the serial port %s from tunnel %i", tun->tunnelId, tun->staticMemory->uart_sm.deviceName, t->tunnelId));
                close_uart_reader(t);
