...
static const uint8_t crcTableHigh[] = {
...
static const uint8_t crcTableLow[] = {
...
bool modbus_rtu_crc_verify_crc_field(uint8_t* buffer, uint16_t length)
{
    ...
    length -= 2; // don't include the CRC field in the CRC calculation

    while (length--)
    {
        index = crcHigh ^ *buffer++;
        crcHigh = crcLow ^ crcTableHigh[index];
        crcLow = crcTableLow[index];
    }
