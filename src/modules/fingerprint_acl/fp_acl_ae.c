...
bool fp_acl_is_connection_allowed(nabto_connect* connection)
{
    ...
    if (!user) {
        ...
        if (connection->isLocal) {
            ...
            NABTO_LOG_INFO(("Unpaired user with fingerprint [%02x:%02x:%02x:%02x:...] is not allowed remote access",
                            connection->fingerprint.value.data[0], connection->fingerprint.value.data[1], connection->fingerprint.value.data[2], connection->fingerprint.value.data[3]));
            return false;
        }
    }
