...
{
    unabto_query_write_uint8_list(write_buffer* queryResponse, string, strlen(string));
}
                                                    ...
                                                    buffer_write_t* write_buffer)
{
    ...
    if (!aclDb.load_settings(&aclSettings) != FP_ACL_OK) {
    ...
    user.fp = request->connection->fingerprint;
    ...
    if (status == FP_ACL_DB_OK) {
        ...
        return write_empty_user(write_buffer, FP_ACL_STATUS_USER_DB_FUL);
    } 
}
